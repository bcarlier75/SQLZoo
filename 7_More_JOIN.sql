...
-- 9#. List the films where 'Harrison Ford' has appeared.The ord field
--     of casting gives the position of the actor. If ord=1 then this actor is
...
GROUP BY yr
HAVING COUNT(title)= (SELECT MAX(Count) FROM (SELECT yr,COUNT(title) AS Count FROM movie JOIN casting ON movie.id=movieid JOIN actor ON actorid=actor.id WHERE name='John Travolta' GROUP BY yr) as titlte)
...
(SELECT movie.id FROM movie JOIN casting ON movie.id = movieid JOIN actor ON actor.id = actorid WHERE actor.name = 'Julie Andrews')

-- 13#. Obtain a list, in alphabetical order, of actors who've had at least 30 starring roles.
SELECT DISTINCT actor.name
FROM casting JOIN movie ON (movie.id = casting.movieid) JOIN actor ON (actor.id = casting.actorid)
WHERE actorid IN (SELECT actorid FROM casting WHERE casting.ord = 1 GROUP BY casting.actorid HAVING COUNT(actorid) >= 30)
ORDER BY name
...
AND movieid IN (SELECT casting.movieid FROM movie JOIN casting ON (casting.movieid = movie.id) JOIN actor ON (casting.actorid = actor.id) WHERE actor.name = 'Art Garfunkel')
