...
-- 6#. Change the query so that the services between 'Craiglockhart' and 'London Road' are
...
JOIN stops stopa ON (a.stop = stopa.id) JOIN stops stopb ON (b.stop = stopb.id)
JOIN stops stopc ON (c.stop = stopc.id) JOIN stops stopd ON (d.stop = stopd.id)
